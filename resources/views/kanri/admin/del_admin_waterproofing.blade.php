...
@section('content')
        <div class="dashboard-container">
            <div class="information-section">
                ...
                <form action="{{ route('ecoulex.kanri.delEditAdminWaterProofInfo') }}" method="post" enctype="multipart/form-data" novalidate>
                @csrf <!-- CSRFトークンを含める -->
                        ...
                        <p class="message"> {{ session('status') }}</p>
                            ...
                            <div class="form-group">
                                ...
                                @foreach($WarterProofCatAll as $WarterProofCat)
                                    ...
                                    <div><input type="checkbox" name="WaterProofingCat[]" value="{{$WarterProofCat -> waterproofcat_id}}" required checked disabled>{{$WarterProofCat -> catName}}</div>
                                    @else
                                    <div><input type="checkbox" name="WaterProofingCat[]" value="{{$WarterProofCat -> waterproofcat_id}}" required disabled>{{$WarterProofCat -> catName}}</div>
                                    @endif
                                @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="waterproofing_job_catch" value="{{$WarterProofs[0] -> waterproofing_job_catch}}" maxlength="30" readonly>
                            </div>
                            <div class="form-group">
                                <div class="form-label">
                                    <label>防水工事用詳細:</label>
                                ...
                                <textarea name="waterproofing_job_description" rows="4" cols="50" maxlength="100" readonly>{{$WarterProofs[0] -> waterproofing_job_description}}</textarea>
                            </div>
